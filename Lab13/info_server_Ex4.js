...
//Rule to handle process_form request form order_page.html
app.post("/process_form", function (request, response) {
   ...
   if(typeof POST ['quantity_textbox'] != 'undefined')
   {
       ...
       if(isNonNegativeInteger(quantity)){
           ...
           response.send(`<H2>Thank you for ordering ${quantity} ${brand}! Your total is \$${quantity * brand_price}!</H2>`);  
       }else{
           response.send(`<I>${quantity} is not a valid quantity!`)
       }
   }
});
